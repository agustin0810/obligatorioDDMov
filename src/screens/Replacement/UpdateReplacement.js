...
const UpdateReplacement = ({ route, navigation }) => {
  ...
  useEffect(() => {
    db.transaction((txn) => {
      ...
      txn.executeSql(`SELECT * FROM replacements WHERE id=?`, [id], (tx, results) => {
        // validar resultado
        if (results.rows.length > 0) {
          ...
          Alert.alert(
            "Mensaje",
            "No se encontró el repuesto con id: "+id,
            [
              {
                text: "Ok",
                onPress: () => navigation.navigate("HomeReplacement"),
  ...
  const updateC = () =>{
    ...
    db.transaction((tx) => {
      ...
      tx.executeSql(
        "UPDATE replacements SET name = ?, amount = ?, treatmentId = ?",
